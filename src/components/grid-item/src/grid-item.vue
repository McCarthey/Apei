<template>
    <a
        ...
        class="vbase-grid"
        :class="{
            'vvpay-grid-item-no-border':
                (isLast && !$parent.showLrBorders) ||
                (!isLast && !$parent.showVerticalDividers)
        }"
        @click="onClick"
        :style="style"
    >
        <div class="vbase-grid__icon" v-if="hasIconSlot || icon">
            <slot name="icon">
                <img :src="icon" alt="" />
            </slot>
        </div>
        <p v-if="hasLabelSlot || label" class="vbase-grid__label">
            <slot name="label">
...
<style lang="less">
	.vbase-grid.vvpay-grid-item-no-border {
		&:before {
			display: none;
		}
	}
