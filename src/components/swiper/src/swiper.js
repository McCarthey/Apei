...
class Swiper {
    constructor(options) {
        this._default = {
            container: ".vvpay-swiper",
            item: ".vvpay-swiper-item",
            direction: "vertical",
            activeClass: "active",
            threshold: 50,
            duration: 300,
        ...
        this.$container = this._options.container.querySelector(
            ".vvpay-swiper"
        );
    ...
    updateItemWidth() {
        this._width =
			this.$box.offsetWidth || document.documentElement.offsetWidth;
        this._distance =
    ...
    _init() {
        this._height =
			this._options.height === "auto" ? "auto" : this._options.height - 0;
    ...
    go(index) {
        ...
        index =
			this._position.indexOf(index) -
			this._position.indexOf(this._current);
    ...
    destroy() {
        ...
        if (this._options.loop && this.count === 2) {
            let $item = this.$container.querySelector(
                `${this._options.item}-clone`
            );
            $item && this.$container.removeChild($item);
            $item = this.$container.querySelector(
                `${this._options.item}-clone`
            );
            $item && this.$container.removeChild($item);
        }
