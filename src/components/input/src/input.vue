<template>
    <div
        class="vp-input vbase-cell"
        :class="{
            'vbase-cell_warn': showWarn,
            disabled: disabled,
            'vp-input-has-right-full': hasRightFullHeightSlot,
        }"
    >
        <div class="vbase-cell__hd">
            <div
                v-if="hasRestrictedLabel"
                :style="labelStyles"
            >
                <slot name="restricted-label" />
            </div>
            <slot name="label">
                <label
                    v-if="title"
        ...
        <div
            class="vbase-cell__bd vbase-cell__primary"
            :class="placeholderAlign ? `vp-input-placeholder-${placeholderAlign}` : ''"
        >
            <input
                v-if="!type || type === 'text'"
                :id="`vp-input-${uuid}`"
        ...
        <div class="vbase-cell__ft">
            ...
            <slot name="right" />
            <div
                v-if="hasRightFullHeightSlot"
                class="vp-input-right-full"
            >
...
import VpIcon from '../../icon/src/icon'
import VpToast from '../../toast/src/toast'
...
export default {
    ...
    computed: {
        ...
        labelClass() {
            return {
                'vp-cell-justify': this.$parent.labelAlign === 'justify' || this.$parent.$parent.labelAlign === 'justify',
            }
        },
        pattern() {
...
<style lang="less">
	...
	@import '../../../styles/vbase/widget/vbase-cell/vbase-form/vbase-vcode';

	.vp-input .vp-input-placeholder-right input::-webkit-input-placeholder {
		text-align: right;
	}
	.vp-input .vp-input-placeholder-center input::-webkit-input-placeholder {
		text-align: center;
	}
	.vp-input .vp-input-icon {
		font-size: 21px;
	}
	.vp-input-icon.vbase-icon-warn:before,
	.vp-input-icon.vbase-icon-success:before {
		font-size: 21px;
	}
	.vp-input .vbase-icon {
		padding-left: 5px;
	}
	.vp-input.vbase-cell_vcode {
		padding-top: 0;
		padding-right: 0;
		padding-bottom: 0;
	}
